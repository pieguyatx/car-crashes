...
% assume y = B0 + B1xt + et
split=104;  % index at which the data "splits" into two parts:
% first part for modeling, 2nd part to test the prediction
datawindow=split*2;

for i2=1:nFULLorig-datawindow-1

    numFULL = numFULLorig(i2:(i2+datawindow-1));
    timeFULL = timeFULLorig(i2:(i2+datawindow-1));
    nFULL = size(numFULL,1);  % data in full model + prediction window
    
    num = numFULLorig(i2:(i2+split-1));  
    time = timeFULLorig(i2:(i2+split-1));
    n = size(num,1);  % data in model window
...
%        noise0 = modelnoise((i-4-MAorder):(i-4),1); % past relevant noise
        ...
        prediction1(i+1,1) = -phis(2:ARorder+1,1)'*flipud(pastdata1) + ...
        ...
        prediction2(i+1,1) = -phis(2,1)*prediction1(i+1,1) + ...
        ...
        if ARorder>1
            prediction3(i+1,1) = -phis(2,1)*prediction2(i+1,1) + ...
                -phis(3,1)*prediction1(i+1,1) + ...
        ...
        if ARorder>2
            prediction4(i+1,1) = -phis(2,1)*prediction3(i+1,1) + ...
                -phis(3,1)*prediction2(i+1,1) + ...
                -phis(4,1)*prediction1(i+1,1) + ...
        ...
        if ARorder>3
            prediction5(i+1,1) = -phis(2,1)*prediction4(i+1,1) + ...
                -phis(3,1)*prediction3(i+1,1) + ...
                -phis(4,1)*prediction2(i+1,1) + ...
                -phis(5,1)*prediction1(i+1,1) + ...
        ...
        predictionupper(i+1,1) = prediction4(i+1,1) + predictionerr;
        predictionlower(i+1,1) = prediction4(i+1,1) - predictionerr;
    end
...
%     title('ARMA(10,9) 5-step ahead prediction; Texas, 2008-2010')
...
%     ylabel('Detrended crashes');
    ...
    i2index(i2,1) = i2;
    ARorder2(i2,1) = ARorder;
    MAorder2(i2,1) = MAorder;
    VarPredErr(i2,1) = varactualprederror;
    
end

%% plot results
plot(i2index,ARorder2,'.-',i2index,MAorder2,'.-')
title('ARMA Model Orders vs. 2-Year Time Window Start')
xlabel('Window start time (week) ')
ylabel('Model order')
legend('AR', 'MA')

plot(i2index,VarPredErr)
title('ARMA Model Prediction Error Variance vs. 2-Year Time Window Start')
xlabel('Window start time (week) ')
ylabel('Variance of Model Prediction Error for next 2 years')

scatter(ARorder2,VarPredErr,'.')
title('ARMA Model Prediction Error Variance vs. Model AR order')
xlabel('AR Order ')
ylabel('Variance of Model Prediction Error for next 2 years')

% freq distribution
hist(ARorder2,[1:18])
title('ARMA Model Order Frequency Distribution, for 2-Year Windows')
xlabel('AR Order')
ylabel('Count')
