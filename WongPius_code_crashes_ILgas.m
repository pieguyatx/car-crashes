...
%   PREDICTION OF A SINGLE MODEL --- choose the model data manually
...
TX_daily = dataraw(:,6);
IL_daily = dataraw(:,7);
[dataraw2,txt,raw] = xlsread('dataGAS.xls') ;  
gas_weekly = dataraw2(:,4);
...
% record F-tests elsewhere!!!
na = 2;  % AR order, 1st argument as output
nb = 2; % AR order+1, 2nd argument as input; 
    % +1 when lag (nk) =1, is for phi0 on x2t
nc = na-1;  % MA order
nk = 1;  % lag =1 for output x2, lag=0 for output=0
dataARMA = iddata(data,gasmodel,1);  % output input
ModelCHECK = armax(dataARMA,[na nb nc nk]); % sys = armax(data,[na nb nc nk])
% present(Model)  % use this command to see the confidence intervals
noiseVarCHECK = ModelCHECK.noisevariance;
%  residualsCHECK = resid(dataARMA,ModelCHECK,'Corr',n);
...
phis2CHECK = [ModelCHECK.b; ModelCHECK.db]';
thetasCHECK = [ModelCHECK.c; ModelCHECK.dc]';
CurrentEstimationInfo=ModelCHECK.EstimationInfo;
RSSCHECK = CurrentEstimationInfo.LossFcn*(n-na-nb-nc-1)
residuals = resid(dataARMA,ModelCHECK,'Corr',n)
%axis([0 n -.5 1])
ModelCHECK
ModelCHECK.noisevariance
...
%% 5-step ahead prediction (revised)
...
dataFULL = iddata(actualdata,gas_weekly,1);
ypred = predict(ModelCHECK,dataFULL,5);
...
for i=1:363
    ...
    predictionupper(i+1,1) = ypred.y(i+1,1) + predictionerr;
    predictionlower(i+1,1) = ypred.y(i+1,1) - predictionerr;
end

plot([1:364],actualdata(1:364),'b',[262:364],ypred.y(262:364),'r',...
    timeFULL(split+1:nFULL), predictionupper(split+1:nFULL),...
    ':k',timeFULL(split+1:nFULL),predictionlower(split+1:nFULL),':k' )
legend('Actual data','Predicted data','95%CI');
title('5-step ahead predictions, 2008-2010')
xlabel('Time (week)')
ylabel('Crashes, detrended')

actualprederror = actualdata(split+1:size(ypred.y))...
    -ypred.y(split+1:size(ypred.y));
varactualprederror = var(actualprederror)

%% (orig) 5-step ahead prediction based on ARMA, code cells above
...
title('ARMA(2,1) 5-step ahead prediction; Texas, 2007-2010')
...
plot(timeFULL,actualdata,timeFULL(split+1:size(prediction5)),...
    ...
    prediction5(split+1:size(prediction5)))
title('1- and 5-step ahead predictions, Jun 2007-2010')
xlabel('Time (week)')
ylabel('Crashes, detrended')
