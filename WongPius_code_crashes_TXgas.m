...
n = size(num,1);
gasTXmodel = gasTX_weekly(1:split);
...
na = 6;  % AR order, output (crashes)
nb = 7; % AR order, input (gas)
nc = na-1;  % MA order
nk = 1;
dataARMA = iddata(data,gasTXmodel,1);
...
dataFULL = iddata(numFULL,gasTX_weekly,1);
ypred = predict(ModelCHECK,dataFULL,5);
plot([1:364],dataFULL,[262:364],ypred);
legend('Actual data','Predicted data');

%% (OLD) 5-step ahead prediction based on ARMA, code cells above
