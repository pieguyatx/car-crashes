...
time = dataraw(:,4);
TX_daily = dataraw(:,6);  % actually this is austin data
% IL_daily = dataraw(:,7);
...
ylabel('Crashes')
%axis([1 n 0 2500 ])
title('Austin monthly crashes, showing linear trend')
...
%axis([1 n -3000 3000 ])
title('Austin monthly crashes, detrended')
...
%axis([0 n -2500 2500 ])
title('Actual vs. Modeled Austin monthly crashes, detrended')
legend('Actual', 'Model')
...
%% Graph actual vs. modeled detrended data (ARMA(n,m)
modeleddata = data-residualsCHECK;
plot(X(:,2),data,'r',X(:,2),modeleddata,'k')
xlabel('Time (months)')
ylabel('Detrended crash count')
%axis([0 n -2500 2500 ])
title('Actual vs. Modeled Texas monthly crashes, detrended')
legend('Actual', 'Model')
strRSS = num2str(RSS1,'%1.3e\n');
str1 = ['Model RSS = ',strRSS];
text(30,-5000,...
